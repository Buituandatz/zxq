...
async def background_task(application: Application):
  while True:
    await application.bot.set_chat_permissions(chat_id=CHAT_ID,permissions=ChatPermissions(can_send_messages=True,can_send_voice_notes=True,can_send_photos=True,can_invite_users=True))
    ...
    if check_cai == []:
      await application.bot.send_message(chat_id=CHAT_ID,text="🤖 BOT SẼ TỰ ĐỘNG LÀM CÁI 👑",parse_mode="HTML")
      ...
      await application.bot.send_message(chat_id=CHAT_ID,text=f"👤 NGƯỜI CHƠI {cai_chon[0][0][:-4]}**** ĐƯỢC CHỌN LÀM CÁI 👑",parse_mode="HTML")
    ...
    await application.bot.set_chat_permissions(chat_id=CHAT_ID,permissions=ChatPermissions(can_send_messages=False,can_send_voice_notes=False,can_send_photos=False,))
    await asyncio.sleep(3)
    datang=database.get_phien_cuoc(get_phien_hien_tai['phien_hien_tai'])
    count_T = sum(1 for item in datang if item[1] == 'T')
    count_X = sum(1 for item in datang if item[1] == 'X')
    message = await application.bot.send_message(chat_id=CHAT_ID,text=f"⌛ Hết thời gian đặt cược! \n\n⚫️ Cửa TÀI: {count_T} người đặt. Tổng tiền {quy_doi(database.get_home_data()['tong_bet_tai'])} ₫ \n⚪️ Cửa XỈU: {count_X} người đặt. Tổng tiền {quy_doi(database.get_home_data()['tong_bet_xiu'])} ₫\n\n🎲🎲🎲 BOT CHUẨN BỊ TUNG XÚC XẮC 🎲🎲🎲",parse_mode="HTML")
    alldice = []
    await asyncio.sleep(2)
    for i in range(3):
      message = await application.bot.send_dice(chat_id=CHAT_ID,emoji="🎲")
